...
def get_reshaped_glyph(target, location):
    if target in PERSIAN_GLYPHS:
        return PERSIAN_GLYPHS[target][location]
    else:
        return target
...
def get_glyph_type(target):
    if target in PERSIAN_GLYPHS:
        return PERSIAN_GLYPHS[target][5]
    else:
        return 2
...
def is_persian_character(target):
    return target in PERSIAN_GLYPHS or target in HARAKAT
...
def has_persian_letters(word):
    for c in word:
        if is_persian_character(c):
            return True
    return False
...
def get_words_from_mixed_word(word):
    ...
    for c in word:
        if is_persian_character(c):
            if temp_word and not is_persian_word(temp_word):
                words.append(temp_word)
                temp_word = c
            else:
                temp_word += c
        else:
            if temp_word and is_persian_word(temp_word):
                words.append(temp_word)
                temp_word = c
            else:
                temp_word += c
